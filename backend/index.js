...
app.delete('/tablets/:tabletsId', async (req, res) => {
    try {
        let tabletsId = parseInt(req.params.tabletsId);
        const [rows, fields] = await db.query('DELETE FROM tablets WHERE id =?', [tabletsId]);
        if (rows.length === 0) {
